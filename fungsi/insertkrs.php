<?php
	...
	?>
	<script>
	alert("Anda Belum Melakukan Pembayaran SPP");
	...
	$sqlmatkul = mysql_query("SELECT makul_matakuliah.kode_makul, makul_matakuliah.nama_makul, makul_matakuliah.sks, makul_matakuliah.semester FROM makul_matakuliah LEFT JOIN student_mahasiswa ON makul_matakuliah.konsentrasi_id=student_mahasiswa.konsentrasi_id where student_mahasiswa.nim='$nim'");
...
<html>
	<head>
		...
		document.getElementById(td1).style.backgroundColor = "#cccccc";
		document.getElementById(td2).style.backgroundColor = "#cccccc";
		document.getElementById(td3).style.backgroundColor = "#cccccc";
		document.getElementById(td4).style.backgroundColor = "#cccccc";
		}else {
	...
	<body>
		...
		<td id="k3<?=$i;?>"><?=$sks[$i];?></td>
		<td id="k4<?=$i;?>">
		<input type="checkbox" name="mk[]" onclick="hitungtotal()" value="<?=$kode_makul[$i];?>" id="mk<?=$i;?>">
		</td>
		</tr>
		<?php
		}
		...
		<td><input type="text" name="totalsks" size="2" maxlength="2" id="jsks" readonly="readonly"></td>
		</tr>
		<tr><td><input type="submit" value="Proses" style="cursor:pointer;"></td></tr>
		</table>
		</form>
		</div>
		</div>
